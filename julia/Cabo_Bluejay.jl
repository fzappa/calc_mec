...
NCfeixe = 6; #Número de condutores por feixe
Dfeixe = 0.657; #Diâmetro do feixe (m)
VAO = 294.30; #Comprimento do vão
...
######################### CATENARIA #########################
#Dados da catenaria
println(@sprintf "Comprimento do cabo para o vão de %5.2f m: %5.2f m" VAO LCABO);
println(@sprintf "H(max) = %5.2f m" H);
println(@sprintf "H(min) = %5.2f m\n\n" minimum(Y));
...
plt.figure(2);
display(plt.scatter(PX,PY,s=200,alpha=0.5));
...
plt.figure(3);
display(plt.scatter(Cx,Cy,s=50,alpha=0.5));
display(plt.title("Configuração do feixe"));
plt.xlabel("Largura [m]");
plt.ylabel("Altura [m]");
plt.grid("on");
